...
namespace MyGL {
    ...
    void SSAOpass::resizeFrame() {
        if (this->out->width() != this->mpass->frameBuffer()->width()
            || (this->out->height() != this->mpass->frameBuffer()->height())) {
            this->out->setFiltration(0LL, 1LL, this->mpass->frameBuffer()->width(),
                                     this->mpass->frameBuffer()->height(), 3LL);
            this->depth->setFiltration(0LL, 3LL, this->mpass->frameBuffer()->width(),
                                       this->mpass->frameBuffer()->height(), 21LL);
        }
    }
