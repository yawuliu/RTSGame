...
namespace MyGL {
    ...
    public:
        ...
        virtual bool drawEvent(IGraphicsObject const &, IMaterial const &); // 9

        virtual bool passEvent(IRenderPass const *); // 10

        virtual void completeDraw(IRenderPass const *);// 11

        virtual bool passEvent(AddBlendPass const *);// 12

        virtual void completeDraw(AddBlendPass const *);// 13

        virtual bool passEvent(ColorPass const *);// 14

        virtual void completeDraw(ColorPass const *);// 15

        virtual bool passEvent(ShadowPass const *);// 16

        virtual void completeDraw(ShadowPass const *);// 17

        virtual bool passEvent(TransparentPass const *);// 18

        virtual void completeDraw(TransparentPass const *);// 19

        virtual bool passEvent(DepthPass const *);// 20

        virtual void completeDraw(DepthPass const *);// 21

        virtual bool passEvent(GlowPass const *);// 22
