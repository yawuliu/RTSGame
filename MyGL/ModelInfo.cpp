...
MyGL::ModelInfo::ModelInfo(MyGL::IIOModel& m) {
	this->updateFrom(m);
}
...
MyGL::IModelInfo* MyGL::ModelInfo::operator=(MyGL::IModelInfo const& m) {
	__m128d v2;
	__m128d v3;
	__m128d v4;
	__m128d v5;
...
double MyGL::ModelInfo::minX() {
	return this->minD[0];
}

double MyGL::ModelInfo::minY() {
	return this->minD[1];
}

double MyGL::ModelInfo::minZ() {
	return this->minD[2];
}

double MyGL::ModelInfo::maxX() {
	return this->maxD[0];
}

double MyGL::ModelInfo::maxY() {
	return this->maxD[1];
}

double MyGL::ModelInfo::maxZ() {
	return this->maxD[2];
}

double MyGL::ModelInfo::radius() {
	return this->r;
}

void MyGL::ModelInfo::updateFrom(MyGL::IIOModel const& m) {
	__int64 v2;
	const double* v3;
	double v4;
	int v5;
	...
	if ((*((unsigned int(__fastcall**)(MyGL::IIOModel* const)) m->_vptr_IIOModel + 18))(m)) {
		for (i_0 = 0; i_0 <= 2; ++i_0) {
			this->minD[i_0] = *(float*)(4LL * i_0
				+ (*((__int64(__fastcall**)(
					MyGL::IIOModel* const)) m->_vptr_IIOModel + 8))(m));
			this->maxD[i_0] = *(float*)(4LL * i_0
				+ (*((__int64(__fastcall**)(
					MyGL::IIOModel* const)) m->_vptr_IIOModel + 8))(m));
		}
		for (i_1 = 0;; i_1 += 3) {
			v5 = (*((__int64(__fastcall**)(MyGL::IIOModel* const)) m->_vptr_IIOModel + 18))(m);
			if (3 * v5 <= i_1)
				break;
			for (r = 0; r <= 2; ++r) {
				v2 = (*((__int64(__fastcall**)(MyGL::IIOModel* const)) m->_vptr_IIOModel + 8))(m);
...
void MyGL::ModelInfo::setBox(float x, float y, float z, float X, float Y, float Z) {
	double dVec[3];
	int i;
