...
int NorwegianWoodStyle::pixelMetric(PixelMetric metric,
	...
	case PM_ScrollBarExtent:
		return 0;// QStyle::pixelMetric(metric, option, widget) + 4;
	default:
		return 0;// QStyle::pixelMetric(metric, option, widget);
	}
}
//! [8]
...
drawCBox(ControlElement element, const QStyleOption* option,
	...
	if (element == CE_CheckBoxLabel) {
		...
		//QStyle::drawControl(element, &opt, painter, widget);
	}
	else {
...
QRect NorwegianWoodStyle::subElementRect(QStyle::SubElement subElement, const QStyleOption* option,
	const QWidget* widget) const {
	return QRect();
}

void NorwegianWoodStyle::drawComplexControl(QStyle::ComplexControl cc, const QStyleOptionComplex* opt, QPainter* p,
	const QWidget* widget) const {

}

QStyle::SubControl
NorwegianWoodStyle::hitTestComplexControl(QStyle::ComplexControl cc, const QStyleOptionComplex* opt, const QPoint& pt,
	const QWidget* widget) const {
	return SC_SpinBoxEditField;
}

QRect
NorwegianWoodStyle::subControlRect(QStyle::ComplexControl cc, const QStyleOptionComplex* opt, QStyle::SubControl sc,
	const QWidget* widget) const {
	return QRect();
}

QSize NorwegianWoodStyle::sizeFromContents(QStyle::ContentsType ct, const QStyleOption* opt, const QSize& contentsSize,
	const QWidget* w) const {
	return QSize();
}

QPixmap NorwegianWoodStyle::standardPixmap(QStyle::StandardPixmap standardPixmap, const QStyleOption* opt,
	const QWidget* widget) const {
	return QPixmap();
}

QIcon NorwegianWoodStyle::standardIcon(QStyle::StandardPixmap standardIcon, const QStyleOption* option,
	const QWidget* widget) const {
	return QIcon();
}

QPixmap
NorwegianWoodStyle::generatedIconPixmap(QIcon::Mode iconMode, const QPixmap& pixmap, const QStyleOption* opt) const {
	return QPixmap();
}

int NorwegianWoodStyle::layoutSpacing(QSizePolicy::ControlType control1, QSizePolicy::ControlType control2,
	Qt::Orientation orientation, const QStyleOption* option,
	const QWidget* widget) const {
	return 0;
}

NorwegianWoodStyle::NorwegianWoodStyle(const QPixmap& buttonBase, const QPixmap buttonBack[],
	const QPixmap& frmBase, const QPixmap& frameBack, const QPixmap& ckBoxCheck,
	const QPixmap& paperBack)
	: buttonBase(buttonBase), frmBase(frmBase), frameBack(frameBack),
	ckBoxCheck(ckBoxCheck), paperBack(paperBack) {
