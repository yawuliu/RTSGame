...
#include <MyGL_XmlData/MyGL_XmlData.h>
...
void GLWidget::setAutoBufferSwap(int)
{
}
	...
	if (isRotation == 1) {
		if (GLObject::ideObject) {
			...
			if (selRect.isStarted() &&
				...
				if (game.controller())
					game.controller()->rectEvent(r);
				else
					game.select(r);
			}
			else {
				if (game.controller())
					game.controller()->mouseLClick(mouseCoordsF[0], mouseCoordsF[1]);
				else
...
void glInit() {
	...
	//    qDebug() << "[OpenGL] OpenGL version: " << reinterpret_cast<const char *>(glf->glGetString(GL_VERSION));
	//    qDebug() << "[OpenGL] OpenGL vendor: " << reinterpret_cast<const char *>(glf->glGetString(GL_VENDOR));
	//    qDebug() << "[OpenGL] OpenGL renderer: " << reinterpret_cast<const char *>(glf->glGetString(GL_RENDERER));
	//    qDebug() << "[OpenGL] OpenGL extensions: " << reinterpret_cast<const char *>(glf->glGetString(GL_EXTENSIONS));
